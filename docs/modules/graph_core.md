...
| `free_edge_count` | `uint64_t` | Count of freed edge chunks (one freelist record per orphaned `(node, relation)` chunk). Bumped both by `delete_node_from_disk` (the deleted node's chunks) and by `update_node_edges` (the node's old chunks on every `add_edge` / inbound cleanup). Maintained since 2026-06-07 ([BUG-017](../legacy/known_bugs.md#2026-06-07--bug-017-update_node_edges-orfanizza-regioni-senza-spingerle-sulla-freelist)). The `rel`/`edges` bins are not reused yet, so the count only grows. |
...
**Side effects on disk (since 2026-05-30):** appends a new `RelationNodeList` to `nodes.dat`, appends a fresh contiguous chunk per relation to `edges.dat`, patches `NodeIndex.relation_offset` in `nodes.idx` in place. `meta.dat` is rewritten on every call (since 2026-06-07 `update_node_edges` bumps `free_edge_count`; a new edge also bumps `next_edge_id`/`edge_count`). The previously persisted `RelationNodeList` region and edge chunks are now pushed onto the `rel`/`edges` freelist bins and zeroed (since 2026-06-07, [BUG-017](../legacy/known_bugs.md#2026-06-07--bug-017-update_node_edges-orfanizza-regioni-senza-spingerle-sulla-freelist) fixed) — they are tracked, though not yet reused. See [BUG-001 fixed](../legacy/known_bugs.md#2026-05-26--bug-001-add_edge-non-persiste-su-disco), [BUG-002 fixed](../legacy/known_bugs.md#2026-05-26--bug-002-edgeid-non-globale-tra-nodi) and the [Edge persistence design decision](../legacy/design_decisions.md#2026-05-30--edge-persistence-append--obsolete--in-place-index-patch).
...
| `void update_node_edges(BaseNode&, MetaRecord&, uint64_t node_id)` | Persists the current `neighborgs` of an already-on-disk node to `nodes.dat` / `edges.dat` and patches `NodeIndex.relation_offset` in `nodes.idx` in place. Since 2026-06-07 ([BUG-017](../legacy/known_bugs.md#2026-06-07--bug-017-update_node_edges-orfanizza-regioni-senza-spingerle-sulla-freelist)) it also pushes the OLD relation-list region onto the `rel` bin and each old edge chunk onto the `edges` bin, zeroes those bytes, and bumps `meta.free_edge_count` (hence the non-`const` `MetaRecord&`). Single source of in-place mutation; called by `Graph::add_edge` and by `Graph::delete_node`'s inbound cleanup. |
