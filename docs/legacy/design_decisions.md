...
- **Conseguenze:**
  - `Graph::insert<ComplexRecord>(...)` non compilava: il path passava per `node_to_record` (`odt/node_odt.h:22`) che ha `static_assert(std::is_trivially_copyable_v<T>)`. Dal 2026-05-30 il problema sul ramo `node_to_record` è risolto via `if constexpr` in `write_node` che salta del tutto la chiamata per `T = ComplexRecord` (vedi [BUG-010](known_bugs.md)). `Graph::insert<ComplexRecord>` resta comunque non compilabile a causa di un nuovo bug separato, [BUG-015](known_bugs.md) (chiamata di `std::to_string` sul payload nel log).
