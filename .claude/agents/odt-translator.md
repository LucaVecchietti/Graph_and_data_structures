...
## Your territory
- `graph_core/odt/node_odt.{h,cpp}`, `graph_core/odt/edge_odt.{h,cpp}` — the bridge between domain (`Node<T>`/`BaseNode`/`EdgeRef`) and POD (`NodeRecord`/`NodeRelationList`/`Edge`/`ComplexHeader`).
...
- `node_to_relation_list(BaseNode&, node_id, head=1)` builds a fixed-width batch header (since 2026-06-19): `batch_size = RELATION_BATCH_TAIL` (constant 2176), `free_bytes = batch_size - type_count*272`. NOT the old variable-width sum — every batch is 2213 B (one `rel` size class). The tail lines are written by the I/O layer (`write_relation_line`/`pad_relation_tail`), not here.
- `edge_to_pod(idx, from, to, weight, prev_offset=0, next_offset=0)` builds the fixed-width `Edge { id, weight, to_node, from_node, prev_offset, next_offset }` (48 B; chain offsets added 2026-06-19, default 0).
...
**Edge attribute payloads (typed/"COMPLEX" edges)** — mirror the COMPLEX node design: an `EdgeHeader { type_label_size, json_file_path_size }` + two length-prefixed strings + a sidecar JSON, with `edge_*` helpers paralleling `complex_node_to_record`/`write_complex`/`read_complex`. Key constraint: `edges.dat` stores fixed-width 48-byte `Edge` records chained per `(node,relation)` via `prev_offset`/`next_offset`, so the payload must stay **out-of-line** (the `Edge` keeps fixed size plus a reference into the attribute store). See `docs/ROADMAP.md`.
